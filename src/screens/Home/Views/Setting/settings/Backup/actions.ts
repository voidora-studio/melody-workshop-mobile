...
import { initSetting, updateSetting } from '@/config/setting'
...
const exportAllList = async(path: string) => {
  ...
  try {
    await handleSaveFile(path + '/melody_list.lxmc', data)
  } catch (error: any) {
    log.error(error.stack)
  }
}
...
export const handleExportSetting = async(path: string) => {
  const data = JSON.parse(JSON.stringify({
    type: 'setting',
    data: settingState.setting,
  }))
  try {
    await handleSaveFile(path + '/melody_setting.lxmc', data)
    toast(global.i18n.t('setting_backup_part_export_list_tip_success'))
  } catch (error: any) {
    log.error(error.stack)
    toast(global.i18n.t('setting_backup_part_export_list_tip_failed') + ': ' + (error.message as string))
  }
}

const importSetting = async(path: string) => {
  let configData: any
  try {
    configData = await handleReadFile(path)
  } catch (error: any) {
  ...
  if (configData.type !== 'setting' || !configData.data) {
  ...
  const result = updateSetting(configData.data)
  settingState.setting = result.setting
  global.state_event.configUpdated(result.updatedSettingKeys, result.updatedSetting)
  toast(global.i18n.t('setting_backup_part_import_list_tip_success'))
}

export const handleImportSetting = (path: string) => {
  toast(global.i18n.t('setting_backup_part_import_list_tip_unzip'))
  void importSetting(path).catch((err: any) => {
    log.error(err)
    toast(global.i18n.t('setting_backup_part_import_list_tip_error'))
  })
}
...
export const handleExportAllData = async(path: string) => {
  const data = JSON.parse(JSON.stringify({
    type: 'allData_v3',
  ...
  try {
    await handleSaveFile(path + '/melody_all_data.lxmc', data)
    toast(global.i18n.t('setting_backup_part_export_list_tip_success'))
  } catch (error: any) {
    log.error(error.stack)
    toast(global.i18n.t('setting_backup_part_export_list_tip_failed') + ': ' + (error.message as string))
...
export const handleImportAllData = (path: string) => {
  toast(global.i18n.t('setting_backup_part_import_list_tip_unzip'))
  void importPlayList(path).finally(() => {
    // Also try to import settings if the file contains them
    void handleReadFile(path).then(async(configData: any) => {
      if (configData.type === 'allData_v3' && configData.setting) {
        const result = updateSetting(configData.setting)
        settingState.setting = result.setting
        global.state_event.configUpdated(result.updatedSettingKeys, result.updatedSetting)
      }
    }).catch(() => {
      // ignore settings import errors
    })
  })
}
