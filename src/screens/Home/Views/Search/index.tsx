...
export default () => {
  ...
  useEffect(() => {
    ...
    const handleNavIdUpdate = (id: string) => {
      if (id == 'nav_search') {
        if (settingState.setting['search.isFocusSearchBox']) {
          headerBarRef.current?.focus()
        }
        return
      }
      const isAutoClear = settingState.setting['search.isAutoClear']
      const isAutoClearInput = settingState.setting['odc.isAutoClearSearchInput']
      const isAutoClearList = settingState.setting['odc.isAutoClearSearchList']
      if (!isAutoClear && !isAutoClearInput && !isAutoClearList) return
      if (isAutoClear || isAutoClearInput) {
        headerBarRef.current?.setText('')
        searchActions.setSearchText('')
      }
      if (isAutoClear || isAutoClearList) {
