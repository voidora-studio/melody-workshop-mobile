...
const List = forwardRef<ListType, ListProps>(({ onShowMenu, onMuiltSelectMode, onSelectAll }, ref) => {
  ...
  const isSaveScrollLocationRef = useRef(isSaveScrollLocation)
  isSaveScrollLocationRef.current = isSaveScrollLocation
  ...
  useEffect(() => {
    ...
    const updateList = (id: string) => {
      ...
      void Promise.all([getListMusics(id), isSaveScrollLocationRef.current ? getListPosition(id) : Promise.resolve(0)]).then(([list, position]) => {
