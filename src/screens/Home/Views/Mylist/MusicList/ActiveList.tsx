...
export default forwardRef<ActiveListType, ActiveListProps>(({ onShowSearchBar, onScrollToTop, onFindDuplicates }, ref) => {
  ...
  return (
    <TouchableOpacity onPress={showList} onLongPress={onScrollToTop} style={{ ...styles.currentList, opacity: visibleBar ? 1 : 0, borderBottomColor: theme['c-border-background'] }}>
      ...
      {onFindDuplicates ? (
        <TouchableOpacity style={styles.currentListBtns} onPress={onFindDuplicates}>
          <Icon color={theme['c-button-font']} name="minus-box" />
        </TouchableOpacity>
      ) : null}
