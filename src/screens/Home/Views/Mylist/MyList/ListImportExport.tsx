...
import { handleExport, handleExportCsv, handleExportTxt, handleImport, handleImportMediaFile } from './listAction'
...
export interface SelectInfo {
  ...
  // listId: string
  // single: boolean
  action: 'import' | 'export' | 'exportTxt' | 'exportCsv' | 'selectFile'
}
...
export interface ListImportExportType {
  ...
  exportTxt: (listInfo: LX.List.MyListInfo, index: number) => void
  exportCsv: (listInfo: LX.List.MyListInfo, index: number) => void
...
export default forwardRef<ListImportExportType, {}>((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    exportTxt(listInfo, index) {
      ...
      if (visible) {
        choosePathRef.current?.show({
          title: global.i18n.t('list_export_txt_desc'),
          dirOnly: true,
        })
      } else {
        setVisible(true)
    ...
    exportCsv(listInfo, index) {
      ...
      if (visible) {
        choosePathRef.current?.show({
          title: global.i18n.t('list_export_csv_desc'),
          dirOnly: true,
        })
      } else {
        setVisible(true)
  ...
  const onConfirmPath = (path: string) => {
    switch (selectInfoRef.current.action) {
      ...
      case 'export':
        ...
        break
      case 'exportTxt':
        handleExportTxt(selectInfoRef.current.listInfo, path)
        break
      case 'exportCsv':
        handleExportCsv(selectInfoRef.current.listInfo, path)
        break
      case 'selectFile':
