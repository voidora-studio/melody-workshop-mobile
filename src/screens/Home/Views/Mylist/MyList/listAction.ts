...
import { type FileType, writeFile } from '@/utils/fs'
...
const exportList = async(listInfo: LX.List.MyListInfo, path: string) => {
  ...
  try {
    await handleSaveFile(`${path}/melody_list_part_${filterFileName(listInfo.name)}.lxmc`, data)
  } catch (error: any) {
    log.error(error.stack)
  }
}
...
const exportListToTxt = async(listInfo: LX.List.MyListInfo, path: string) => {
  const musics = await getListMusics(listInfo.id)
  const lines = musics.map(m => `${m.name} - ${m.singer}`)
  const content = lines.join('\n')
  try {
    await writeFile(`${path}/melody_list_${filterFileName(listInfo.name)}.txt`, content)
  } catch (error: any) {
    log.error(error.stack)
    throw error
  }
...
export const handleExportTxt = (listInfo: LX.List.MyListInfo, path: string) => {
  toast(global.i18n.t('setting_backup_part_export_list_tip_zip'))
  exportListToTxt(listInfo, path).then(() => {
    toast(global.i18n.t('setting_backup_part_export_list_tip_success'))
  }).catch((err: any) => {
    log.error(err.message)
    toast(global.i18n.t('setting_backup_part_export_list_tip_failed') + ': ' + (err.message as string))
...
const exportListToCsv = async(listInfo: LX.List.MyListInfo, path: string) => {
  const musics = await getListMusics(listInfo.id)
  const header = 'Name,Singer,Album,Interval'
  const rows = musics.map(m => `"${m.name}","${m.singer}","${m.meta.albumName ?? ''}","${m.interval ?? ''}"`)
  const content = [header, ...rows].join('\n')
  try {
    await writeFile(`${path}/melody_list_${filterFileName(listInfo.name)}.csv`, content)
  } catch (error: any) {
    log.error(error.stack)
    throw error
  }
...
export const handleExportCsv = (listInfo: LX.List.MyListInfo, path: string) => {
  toast(global.i18n.t('setting_backup_part_export_list_tip_zip'))
  exportListToCsv(listInfo, path).then(() => {
    toast(global.i18n.t('setting_backup_part_export_list_tip_success'))
  }).catch((err: any) => {
    log.error(err.message)
    toast(global.i18n.t('setting_backup_part_export_list_tip_failed') + ': ' + (err.message as string))
