...
export default () => {
  ...
  const displayLines = useMemo(() => {
    if (isLxlyricEnabled()) {
      ...
      console.log('[Lyric] isLxlyricEnabled=true, lxLines.length:', lxLines.length)
      if (lxLines.length > 0) {
        const result = lxLines.map(lxLine => ({
          time: lxLine.time,
          text: lxLine.words.map(w => w.text).join(''),
          words: lxLine.words,
          extendedLyrics: [] as string[],
        }))
        console.log('[Lyric] displayLines from lxlyric, count:', result.length, 'first:', result[0]?.text?.substring(0, 20))
        return result
      }
    }
    console.log('[Lyric] using LRC lines, count:', lyricLines.length)
    return lyricLines
  }, [lyricLines])
  ...
  const activeLineWords = useMemo(() => {
    ...
    // Real lxlyric word data
    if (lineData?.words?.length) return lineData.words as WordData[]
    // Uniform fallback: distribute line duration across all characters
